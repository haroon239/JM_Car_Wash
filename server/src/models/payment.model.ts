...
export async function findPayments() {
  return (
    await requireDatabase().query(
      `SELECT pay.id,pay.invoice_id AS "invoiceId",pay.amount,pay.method,pay.reference,pay.paid_at AS "paidAt",pay.recorded_by AS "recordedBy",i.invoice_number AS "invoiceNumber",c.name AS "customerName" FROM payments pay JOIN invoices i ON i.id=pay.invoice_id JOIN customers c ON c.id=i.customer_id ORDER BY pay.paid_at DESC,pay.id DESC`,
    )
  ).rows;
}
...
export async function createPayment(input: PaymentInput) {
  ...
  try {
    ...
    const result = await client.query(
      `INSERT INTO payments (invoice_id,amount,method,reference) VALUES ($1,$2,$3,NULLIF($4,'')) RETURNING id,invoice_id AS "invoiceId",amount,method,reference,paid_at AS "paidAt",recorded_by AS "recordedBy"`,

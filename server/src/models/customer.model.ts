...
export async function archiveCustomer(id: number) {
  return (await requireDatabase().query(
    "UPDATE customers SET deleted_at=NOW(),status='archived' WHERE id=$1 AND deleted_at IS NULL RETURNING id", [id]
  )).rows[0];
}

export async function restoreCustomer(id: number) {
  return (await requireDatabase().query(
    "UPDATE customers SET deleted_at=NULL,status='active',updated_at=NOW() WHERE id=$1 AND deleted_at IS NOT NULL RETURNING id", [id]
  )).rows[0];
}
